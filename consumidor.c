...
int main(int argc, char const *argv[]) {
  ...
  numero_convertido = htonl(0); // 0 para consumir
  ...
  numero_convertido = htonl(vezes); // vezes que vai consumir por ciclo
  if(send(sockfd, &numero_convertido, sizeof(numero_convertido), 0) < 0) {
    fprintf(stderr, "ERRO: Nao foi possivel enviar mensagem ao servidor.\n");
    close(sockfd);
    exit(1);
  ...
  while(1) {
    ...
    if(n == 1) {
      for(i = 0; i < vezes; i++) {
        ...
        n = ntohl(numero_convertido);
        if(n == 0) {
          printf("[PID: %d] Pilha esvaziou! O lote %d teve %d unidades\n", getpid(), i);
          break;
        } else {
          printf("[PID: %d] Consumindo: %d - Lote: %d\n", getpid(), ntohl(numero_convertido), j);
        }
      }
    } else {
      break;
